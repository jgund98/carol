...
export default async function Home() {
  ...
  const book1 = bySlug(works, books[0].slug);
  const book2 = bySlug(works, books[1].slug);
  const leans = [
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="wrap section relative">
          ...
          <div className="mt-14 flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between lg:gap-0">
            {featured.map((c, i) => {
              const w = (c.hero && bySlug(works, c.hero)) || inCollection(works, c.id)[0];
              if (!w) return null;
              const L = leans[i];
              return (
      ...
      <section className="bg-paper">
        <GalleryWall works={wall} total={works.filter((w) => w.kind !== "book").length} />
      </section>
          ...
          <Reveal className="grid gap-6 sm:grid-cols-[0.8fr_1fr] sm:items-center" delay={120}>
            <div className="relative aspect-[9/16]">
              <div className="wrap-edge absolute left-0 top-[8%] w-[72%] rotate-[-6deg] overflow-hidden bg-linen" style={{ aspectRatio: "1" }}>
                <Image src={book1 ? img(book1) : "/art/flower-power-coffee-table-book.jpg"} alt="Flower Power, the paintings of Carol Calicchio" fill sizes="30vw" className="object-cover" />
              </div>
              <div className="wrap-edge absolute bottom-[8%] right-0 w-[72%] rotate-[5deg] overflow-hidden bg-linen" style={{ aspectRatio: "1" }}>
                <Image src={book2 ? img(book2) : "/art/ocean-power-coffee-table-book.jpg"} alt="Ocean Power, the paintings of Carol Calicchio" fill sizes="30vw" className="object-cover" />
              </div>
            </div>
            <div>
