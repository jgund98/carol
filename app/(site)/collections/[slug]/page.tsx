...
// New collections Carol creates are rendered on first visit and cached until
// the office revalidates the site.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { collections } = await getCatalog();
  const c = collectionBySlug(collections, slug);
  if (!c) return {};
  return {
...
export default async function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const { works: all, collections } = await getCatalog();
  const c = collectionBySlug(collections, slug);
  if (!c) notFound();
  const works = inCollection(all, c.id);
  ...
  return (
    <>
      ...
      <section className="wrap pb-24">
        {works.length === 0 && <p className="text-ink/60">Nothing hangs in this collection yet. New work is on the way.</p>}
