...
// Pieces Carol adds in the office render on first visit; edits revalidate the page.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const w = bySlug((await getCatalog()).works, slug);
  if (!w) return {};
  const d = dims(w);
  return {
...
export default async function WorkPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const w = bySlug(works, slug);
  if (!w) notFound();
  const colls = collectionsOf(collections, w);
  const rel = related(works, w, 4);
  const d = dims(w);
