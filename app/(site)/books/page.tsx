            ...
            return (
              <Reveal key={b.slug} className={`grid items-center gap-10 lg:grid-cols-2 ${i % 2 ? "lg:[&>*:first-child]:order-2" : ""}`}>
                <div className="wrap-edge relative mx-auto aspect-square w-full max-w-[520px] overflow-hidden bg-linen">
                  <Image src={w ? img(w) : `/art/${b.slug}.jpg`} alt={`${b.title}: ${b.subtitle}`} fill priority={i === 0} sizes="(min-width:1024px) 45vw, 100vw" className="object-cover" />
                </div>
                <div>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    {w && <span className="display text-2xl">{money(w.price)}</span>}
                    {w && w.available ? (
                      ...
                      <>
                        ...
                        <Link href={`/contact?about=${b.slug}`} className="btn btn-line">Ask for a signed copy</Link>
                      </>
                    )}
                  </div>
                  {w && !w.available && <p className="mt-3 text-xs text-muted">Currently out of stock in the studio shop.</p>}
                </div>
              </Reveal>
            );
          })}
